...
# np.random.seed(SEED)
seed_everything(2334) # <-- Add this line
...
def train(hparams, ModuleClass, ModelClass, DatasetClass, logger):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        monitor=hparams.early_stopping_metric,
        mode='max',
        filename=f'{hparams.name}' + '-{epoch:02d}-{'+f'{hparams.early_stopping_metric}' + ':.2f}'
    )#george 
    ...
    trainer = Trainer(
        ...
        gpus=hparams.gpus, #george
        # accelerator='gpu' if hparams.gpus else 'cpu',
        #accelerator='ddp', #george 
        logger=logger,
        ...
        default_root_dir=str(hparams.output_path),
        ) #george
    # trainer = Trainer(
    #     gpus=hparams.gpus,
    #     logger=logger,
