...
from torchmetrics.classification import MultilabelFBetaScore #george 
# #from torchmetrics import F1Score #george
...
class FeatureExtraction(LightningModule):
    ...
    def init_metrics(self):
        ...
        if self.num_tasks == 2:
            ...
            self.val_f1_tool = MultilabelFBetaScore(num_labels=7, beta=1.0, average='macro') #george
            self.train_f1_tool = MultilabelFBetaScore(num_labels=7, beta=1.0, average='macro') #george
            # self.val_f1_tool = F1Score(task="multilabel", num_labels=7, average='macro')
            # self.train_f1_tool = F1Score(task="multilabel", num_labels=7, average='macro')
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.num_tasks == 2:
            ...
            acc_tool = self.train_acc_tool(p_tool, tool_labels)
            self.log("train_acc_tool", acc_tool, on_epoch=True, on_step=False)

            f1_tool = self.train_f1_tool(p_tool, tool_labels)
            self.log("train_f1_tool", f1_tool, on_epoch=True, on_step=False)

        acc_phase = self.train_acc_phase(p_phase, y_phase)
        self.log("train_acc_phase", acc_phase, on_epoch=True, on_step=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.num_tasks == 2:
            tool_labels = torch.stack(y_tool, dim=1)
            acc_tool = self.val_acc_tool(p_tool, tool_labels)
            self.log("val_acc_tool", acc_tool, on_epoch=True, on_step=False)

            f1_tool = self.val_f1_tool(p_tool, tool_labels)
            self.log("val_f1_tool", f1_tool, on_epoch=True, on_step=False)

        acc_phase = self.val_acc_phase(p_phase, y_phase)
        self.log("val_acc_phase", acc_phase, on_epoch=True, on_step=False)
    ...
    def test_step(self, batch, batch_idx):
        ...
        acc_phase = self.test_acc_phase(y_hat, y_phase) #george
        self.log("test_acc_phase", acc_phase, on_epoch=True, on_step=False) #george
